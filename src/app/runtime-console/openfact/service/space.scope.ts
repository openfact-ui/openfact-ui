...
@Injectable()
export class SpaceScope {

    public space: Observable<string>;

    constructor(protected activatedRoute: ActivatedRoute, protected router: Router) {
        this.space = this.router.events
            ...
            .map((params) => this.getSpace(params))
            .filter((n) => n)
            .distinctUntilChanged();
    }

    protected getSpace(params) {
    ...
    currentSpace() {
        return this.findParamsFor(this.router.routerState.snapshot.root, 'space');
    }
}
