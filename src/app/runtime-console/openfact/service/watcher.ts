...
export class Watcher<L> {
  ...
  protected onWebSocketError() {
    // if (this.retries < 3
    //  && this.connectTime
    //  && (new Date().getTime() - this.connectTime) > 5000) {
    if (this.retries < 3) {
      this.retries = this.retries + 1;
      this.recreate();
    } else {
      console.log('WebSocket for ' + this.pathFn()
        + ' error, retry #' + this.retries + ' so switching to polling mode');
      this.closeWebSocket();
